class GAN():
  def __init__(self,config = None):
    self.Name = "BEGAN" # Boundary Equilibrium GAN
    self.k = 0 # Ressource https://medium.com/@jonathan_hui/gan-energy-based-gan-ebgan-boundary-equilibrium-gan-began-4662cceb7824
    self.Lambda = 0.001
    self.Gamma = 1
    # Input Shape
    self.ImgHeight = 28 
  ...
  def DiscriminatorLoss(self, RealOutput, FakeOutput):
    return tf.reduce_mean(tf.math.add(RealOutput, self.k * FakeOutput))
  
  # Training Procedure
  @tf.function
  def TrainingStep(self, RealImages, k, Gamma):
    ...
    with tf.GradientTape() as GenTape, tf.GradientTape() as DiscTape:
      ...
      # Update Parameter k 
      k = k + self.Lambda * tf.subtract(Gamma * RealOutput, FakeOutput)
      Gamma = Gamma * 0.9999
    ...
    return (DiscLoss, GenLoss, k, Gamma)
  ...
  def Training(self, RealDataset, Normalization = "norm"):
    ...
    for Epoch in tqdm(range(self.Epochs)):
      for BatchNumber in range(len(RealDataset)// self.BatchSize):      
        ...
        self.DiscLoss, self.GenLoss, self.k, self.Gamma = self.TrainingStep(RealImages, self.k, self.Gamma)
