class GAN():
  ...
  # Loss Functions
  def WassersteinLoss(self,Labels,Prediction):
    return - tf.reduce_mean(Labels * Prediction)
  
  # Training Procedure
  @tf.function
  def GenTrainingStep(self,RealImages):
  ...
  def DiscTrainStep(self,RealImages):
    ...
    Noise = np.random.random(size = (self.BatchSize, self.LatentDim))
    # Sample a Random Scaling Factor
    Epsilon = np.random.random(1)
    # Train Discriminator Model on Real and Generated
    with tf.GradientTape() as DiscTape:
      # Interpolation Tape
      with tf.GradientTape() as IntTape:
        # Generate Images from Generator with Noise
        GeneratedImages = self.Generator(Noise,training = True)
        # interpolate Real and Generated Images
        Interpolated = Epsilon * GeneratedImages + (1 - Epsilon) * RealImages

        # Calculate Discriminator Outputs
        RealOutput = self.Discriminator(RealImages, training = True)
        FakeOutput = self.Discriminator(GeneratedImages, training = True)
        IntOutput = self.Discriminator(Interpolated, training = True)

      # Calculate Gradients from "Recorded Operations"
      GradInterpolated = IntTape.gradient(IntOutput, Interpolated)
      
      # Calculate Loss with Gradient Penalty Regularization
      Regularization = tf.reduce_mean(tf.square(tf.norm(GradInterpolated) - 1))
      DiscLoss = self.WassersteinLoss(tf.ones_like(RealOutput),RealOutput) + \
                 self.WassersteinLoss(- tf.ones_like(FakeOutput),FakeOutput) + \
                 Regularization
    ...
    for Epoch in tqdm(range(self.Epochs)):
      for BatchNumber in range(len(RealDataset)// self.BatchSize):     
        ...
        for Step in range(self.Ncritic - 1):
          ...
          self.DiscLoss = self.DiscTrainStep(RealImages)

        # Generator Training      
        self.GenLoss = self.GenTrainingStep(RealImages)
