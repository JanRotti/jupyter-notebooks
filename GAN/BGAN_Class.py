class GAN():
  def __init__(self,config = None):
    self.Name = "BGAN" # Boundary Seeking GAN
    # Input Shape
    self.ImgHeight = 28 
    self.ImgWidth = 28
    self.ImgChannels = 1
  ...
  def CreateGenerator(self):
    ...
    self.Generator.add(Conv2DTranspose(self.GenFMaps / 2, 4, strides = 1,
                                  padding = "same"))
    
    self.Generator.add(BatchNormalization())
    self.Generator.add(LeakyReLU(alpha = 0.2))
    self.Generator.add(Conv2DTranspose(self.GenFMaps / 4, 4, strides = 2,
                                  padding = "same"))
  ...
  # Loss Functions
  def BoundaryLoss(self, PredLabels):
    """
  ...
  def DiscriminatorLoss(self, RealOutput, FakeOutput):
    return - tf.reduce_mean(tf.math.log(RealOutput) + tf.math.log(1 - FakeOutput))
  ...
  def TrainingStep(self,RealImages):
    ...
    with tf.GradientTape() as GenTape, tf.GradientTape() as DiscTape:
      ...
      GenLoss = self.BoundaryLoss(FakeOutput) 
